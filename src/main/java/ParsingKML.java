...
public class ParsingKML {
     ...
     * 返回值：Json
     */
    public String readFile(String modelID, String filePath, String saveFileSting) {
        // 第一步 得到该文件下所有文件名
...
//                hdfs.hdfsUploadFile(inHdfs, fileExtName, hash);
            ...
            // 将文件改名存到另外的位置
            try {
                if (Files.exists(Paths.get(saveFileSting + File.separator + SID + fileExtName))) {
                    continue;
                }
                Files.copy(Paths.get(absoluteS3mFilePath), Paths.get(saveFileSting + File.separator + SID + fileExtName));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        return "the save file location is : " + saveFileSting;
    }
